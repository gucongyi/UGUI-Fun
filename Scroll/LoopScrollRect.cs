...
namespace UnityEngine.UI
{
    ...
    public abstract class LoopScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        public Action<GameObject> getObjectAction;
        //==========LoopScrollRect==========
        ...
        public void ScrollToCellImmediately(int index)
        {
            bool needMoving = true;
            while (needMoving)
            {
                ...
                else
                {
                    ...
                    float maxMove = 100;// Time.deltaTime * speed;
                    if (Mathf.Abs(offset) < maxMove)
                    {
                        needMoving = false;
                        move = offset;
        ...
        public void RefreshCells()
        {
            if (Application.isPlaying && this.isActiveAndEnabled)
            {
                ...
                for (int i = 0; i < content.childCount; i++)
                {
                    ...
                    {
                        content.GetChild(i).gameObject.GetComponent<ScrollIndex>().ScrollCellIndex(itemTypeEnd);
        ...
        {            
            ...
            nextItem.gameObject.name = string.Format($"ScrollIndex{itemIdx}");
            getObjectAction?.Invoke(nextItem.gameObject);
            nextItem.transform.SetParent(content, false);
            nextItem.gameObject.SetActive(true);
            nextItem.gameObject.GetComponent<ScrollIndex>().ScrollCellIndex(itemIdx);
            //dataSource.ProvideData(nextItem, itemIdx);
            return nextItem;
        }
        private void ClearGetObjectAction()
        {
            getObjectAction = null;
        }
        protected override void OnDestroy()
        {
            base.OnDestroy();
            ClearGetObjectAction();
        }
        //==========LoopScrollRect==========
