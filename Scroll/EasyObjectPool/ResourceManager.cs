...
namespace SG
{
    ...
    public class ResourceManager : MonoBehaviour
    {
        ...
        public void InitPool(string poolName,GameObject templateGo, int size, PoolInflationType type = PoolInflationType.DOUBLE)
        {
            ...
            {
                GameObject pb = GameObject.Instantiate(templateGo);
                if (pb == null)
                {
        ...
        /// <returns></returns>
        public GameObject GetObjectFromPool(string poolName,GameObject templateGo, bool autoActive = true, int autoCreate = 0)
        {
            GameObject result = null;

            if (!poolDict.ContainsKey(poolName) && autoCreate > 0)
            {
                InitPool(poolName, templateGo, autoCreate, PoolInflationType.INCREMENT);
            }
